...
#define APP_VERSION "Version:1.2.1"
...
//    ui->model_2nd->addItem("None");
    ...
    db.setDatabaseName("./Configuration/NCC.db");
    if (!db.open())
    {
...
{
    if (ui->model_1st->currentText() == "None" or ui->model_1st->currentText() == NULL)
    {
        ui->model_2nd->clear();
        ui->model_2nd->addItem("None");
    }
...
//        ret=load_fw("./Configuration/moviUsbBoot",NULL);
        ...
        //    int sensorModeId = 1; //4K模式
            ...
            case(10):
                ...
                break;
//            case(11):
//                ui->log_area->append("model 11");
//                strcpy(blob, "./Configuration/blob/mask_detect.blob");
//                ret = sdk_init(NULL,NULL, blob, &cam_info, sizeof(cam_info));
//                fun = mask_show_img_func;
//                break;
            default:
...
//        frameSpecOut* spec =(frameSpecOut*) max_stream_buffer;
    ...
    fun(Postframe, cameraCfg.camWidth, cameraCfg.camHeight, scale,winhndl, true, &cam_info, metadata+sizeof(frameSpecOut)+OUTPUT_INDEX_SIZE,min_score);
    return 0;
}
