...
//获取camera属性
void camera_control_get_features(SensorModesList* list);

//获取设备初始化状态
void camera_control_get_device_init_status(BlobInitStatus *status);

//设置af工作模式
void camera_control_af_mode(camera_ctrl_af_mode af_mode);

//设置lens位移
void camera_control_lens_move(uint32_t lens_position);

//触发af对焦
void camera_control_focus_trigger(void);

//设置camera曝光模式
void camera_control_ae_mode(camera_ctrl_ae_mode flash_mode);

//设置曝光时间，单位为us, 值不大于 1/fps 。
void camera_control_ae_set_exp( uint32_t exp_compensation);

//设置增益
void camera_control_ae_set_gain( uint32_t iso_val);

//设置camera白平衡模式
void camera_control_awb_mode(camera_ctrl_awb_mode awb_mode);

//激活看门狗和复位
void camera_wd_active(void);

//设备重启
void camera_wd_reset(void);

//yuv420 输出控制
void camera_yuv420_out(camera_ctrl_yuv420_out mode);

//h26x 输出控制
void camera_h26x_out(camera_ctrl_h26x_out mode);

//mjpeg 输出控制
void camera_mjpeg_out(camera_ctrl_mjpeg_out mode);

//select camera sernsor
void camera_select_sensor(int sensorid);
